...
Given /^I enter (.+) into field (\d+) of the application$/ do |value, field|
  # Get a textfield by index
  textfield(field.to_i).type value
end

Given "I enter $value into a field of the application showing $text" do |value, field|
...
Then "The view title was $title" do |title|
  navBarTitle = "//UIAApplication[1]/UIAWindow[1]/UIANavigationBar[1]/UIAStaticText[1]"
  find_element(:xpath, navBarTitle).label.should eq title
end

Then "The modal view title was $title" do |title|
  element = find_element(:xpath, "//UIAApplication[1]/UIAWindow[1]/UIAStaticText[1]")
  element.label.should eq title
end
